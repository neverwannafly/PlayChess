...
def make_session_permanent():
    ...
    if session.get('admin_username'):
        ADMIN_DICT['current_admin_'+str(session['admin_username'])] = site_admin.loadAdmin(db, session['admin_username'])
...
def login():
    if request.method=='POST':
        current_admin = site_admin.loadAdmin(db, request.form['admin_username'])
        if current_admin:
