...
def login():
    if request.method=='POST':
        user = site_user.loadUser(users, request.form['username'])
    ...
    if valid_email and valid_username:
        ...
        if existing_email is None and existing_user is None:
            if request.form['password']==request.form['confirm_password']:
                ...
                site_user.addNewUserToDatabase(
                ...
                current_user = site_user.loadUser(users, request.form['username'])[0]
...
@decorators.logout_required
def verify(username):
    current_user = site_user.loadUser(users, username)[0]
    if request.method=='POST':
